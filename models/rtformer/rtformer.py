...
from utils import load_entire_model
from transformer_utils import (DropPath, Identity)
from param_init import (constant_init, kaiming_normal_init, trunc_normal_init)
...
class RTFormer(nn.Layer):
    ...
    def forward(self, x):
        ...
        x_out = self.seghead(paddle.cat([x5_, x6], axis=1))  # 4c, 1/8
        logit_list = [x_out]
